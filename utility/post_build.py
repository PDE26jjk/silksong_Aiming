...
def main():
    # 配置区域 - 根据你的环境修改这些路径
    debug_dll = r"R:\git\game_plugin\silksong_Aiming\obj\Debug\silksong_Aiming.dll"
    release_dll = r"R:\git\game_plugin\silksong_Aiming\obj\Release\silksong_Aiming.dll"
    ...
    # 解析命令行参数
    build_type = "debug"  # 默认使用 debug 版本
    if len(sys.argv) > 1:
        ...
        if arg in ["-d", "--debug"]:
            ...
            print(f"警告: 未知参数 '{arg}'，使用默认配置 (debug)")

    print(f"\n使用 {build_type.upper()} 版本部署")
    ...
    try:
        # 根据构建类型选择源文件
        source_dll = debug_dll if build_type == "debug" else release_dll
